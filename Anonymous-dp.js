...
document.addEventListener("DOMContentLoaded", function () {
  ...
  function modeWithInitials() {
    ...
    const fontColorMatch = canvas.className.match(/profile-font-color-(\d+)/);
    if (fontColorMatch) {
      console.log(fontColorMatch);
  ...
  // Generate random pattern with initials
  function generatePatternWithInitials() {
    // Clear canvas
    ctx.clearRect(0, 0, width, height);
  ...
  function rgbToHsl(rgb) {
    ...
    var g =
      ...
      255;
    var b =
      parseInt(rgb.substring(rgb.lastIndexOf(",") + 2, rgb.length - 1), 10) /
      255;
